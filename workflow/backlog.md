...
### 2026-04-20 · v3.4 内容校准 + CSS 扩展（Dev-H5 部分交付）

**来源**：CEO 2026-04-20 直派（v3.4 任务单 5 项：4 项 data 校准 + 1 项 CSS 新 class）

**状态**：[DONE:Dev-H5 v3.4 · 部分] commit `ebc8707` · 2/5 done + 2/5 done (era) + 1/5 blocked + 1/5 blocked

| # | 任务 | 状态 | 说明 |
|---|---|---|---|
| 1 | 墨西哥城大教堂 ticket + tips[] 插入（照抄 PRD §O-01） | 🔴 **BLOCKED** | PRD §附录 O 不存在（grep 全文无 §O-01/§O-04/附录 O 标记）；PM 必须先交付 O-01 内容，参见 `ux-live-site-diagnostic-2026-04-19-hotfix.md` P0-2 判定"PM 范围，Dev 不能补" |
| 2 | 吴哥窟 wikiImage 换 Angkor_Wat_from_north_pond.JPG | ✅ [DONE:Dev-H5] | CEO 首选 `2/21/Angkor_Wat_temple.jpg` → 404；迭代 3 次后选 `6/6c/Angkor_Wat_from_north_pond.JPG`（2447×1688，1920px thumb 741KB，5塔轮廓+荷塘倒影 postcard view，验证通过） |
| 3 | 伏见稻荷 era: 奈良·711年 → 奈良时代·711年 | ✅ [DONE:Dev-H5] | data.js:662 |
| 4 | 清水寺 era: 平安·778年 → 奈良末·778年 | ✅ [DONE:Dev-H5] | data.js:812 |
| 5 | css/styles.css 新增 `.expanded-section` / `.expanded-section-label` / `.expanded-section-body` 3 class | 🔴 **BLOCKED** | PRD §附录 O 不存在，CSS 规格（颜色 / 字号 / padding / layout）无锚；Dev 不自行发挥 |

**Dev-H5 摘要**：
- 改动：`js/data.js`（3 处），commit `ebc8707` on `origin/dev`
- 自测：Angkor 1920px JPG 200 OK（741KB）+ 浏览器视觉 check 5塔轮廓+水面倒影清晰
- Blockers：2 条（见下）

**Blockers（共 2 条）**：

### [B-01] 严重度：high
- 现象：v3.4 任务单 item [1] 要求"照抄 PRD §O-01" 把墨西哥城大教堂的 ticket + tips[] 插入 data.js:2618，但 PRD §附录 O 在 PRD-travel-h5-v2.md 全文不存在
- 根因：PRD 缺整块内容（ticket 4 必填 + tips 4+ 条，每条遵循 E-01/E-02 schema）
- 建议：PM 补完 §附录 O-01（参照已交付的 特奥蒂瓦坎 / Cairo 模板），再让 Dev-H5 粘入
- 是否阻塞 QA：是（D2 红线 O-06 要求 landmarks[0..2] ticket 4 必填 + tips ≥4 条，墨西哥城大教堂当前是唯一景点 = landmarks[0]，FAIL）

### [B-02] 严重度：high
- 现象：v3.4 任务单 item [5] 要求按 PRD §O-04 新增 `.expanded-section` / `.expanded-section-label` / `.expanded-section-body` 3 个 CSS class，但 PRD §附录 O-04 不存在
- 根因：CSS 规格（颜色 token / 字号 / letter-spacing / padding / 布局位置 / HTML 模板应用在哪个 renderer）全部未定义
- 建议：PM 补完 §附录 O-04 规格表 + HTML 应用示例，再让 Dev-H5 落地
- 是否阻塞 QA：否（新 class 不影响现有渲染；CSS 类增加但无引用点 = 死代码，可放到下一轮）
