...
**不限每 session 跑几个对象，但每个对象必须达到深度下限才能进入下一个。**

### 质量闸门（资料采集完、写长文之前）

资料采集完后，**开始写长文之前**，必须先自检并报给 CEO：
```
📊 质量自检：
- 一手源（真抓到正文）：N 个页面
- 二手源（分析文章/评论/搜索摘要）：M 个
- 抓取失败：K 个
- 证据强度：强（≥3 独立一手源）/ 中（1-2 一手 + 二手补）/ 弱（全二手）
```
- **强** → 继续写长文
- **中** → 继续，但长文里必须标注"二手源补充段落"让 CEO 知道哪些不够硬
- **弱** → **禁止硬写**。停下来报 CEO："⚠️ 一手源不足，建议换抓取方式 / CEO 贴原文补充 / skip。"

### 深度下限（每个研究对象必须满足才算完成）

- 长文 ≥ 150 行（低于这个说明没挖深）
- 一手源 ≥ 3 个页面的正文被真实抓取
- 镜头/条目每条必须有引文佐证（不允许"通用观察"充数）
- 如果 WebFetch 某对象连续失败 → **不跳过，换方式**（Chrome in MCP / CEO 贴原文 / 换同域替代对象），直到拿到足够一手源

## 研究执行中的额外 CEO 介入点

除了闸门 A / B / 质量闸门之外，**只有**这些情况才在研究过程中打断 CEO：
